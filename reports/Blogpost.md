...
In the last years, climate change has become a present topic in the worldwide news. It affects all our lives in multiple areas. One popular effect is global warming. Due to human-induced greenhouse gas emissions, the average temperature will rise in the lower levels of the atmosphere<sup>[1]</sup>. With a rising average temperature drought periods will happen more often. Especially in countries with a higher average temperature than the average in Germany this leads to further problems. In particular, the field of agriculture can be pointed out.
The weather forecast is often only accurate for the next few days. What if there was a method to predict the rough drought level for a longer timespan? This is where our project can help.

## Dataset

We used a Dataset from Kaggle which contained weather data from the US from the years of 2000 until 2020<sup>[2]</sup>. In the dataset, the feature variable was a score ranging from 0 (no drought) to 5 (D4). With D4 being the highest level of drought. There are six levels of drought possible<sup>[3]</sup>. This dataset was constructed with data from the US Drought Monitor and the NASA POWER Project.  
...
In the Kaggle dataset, we were given a "train time series" which contains roughly 16 years of data. Due to the sheer size of this time series, we decided to cut the dataset down to two years for our first explorations and the first model testings. So we did all the following explorations with the years 2000 until 2002.  
The dataset consists of 21 different columns, with one of them being a date column, which we used as the index column in our exploration.  
...
| T2M_RANGE   | Temperature Range at 2 Meters (ºC)     |
| WS10M       | Wind Speed at 10 Meters (m/s)         |
| T2M         | Temperature at 2 Meters (ºC)           |
| WS50M_MIN   | Minimum Wind Speed at 50 Meters (m/s)  |
| T2M_MAX     | Maximum Temperature at 2 Meters (ºC)   |
| WS50M       | Wind Speed at 50 Meters (m/s)         |
| TS          | Earth Skin Temperature (ºC)            |
| WS50M_RANGE | Wind Speed Range at 50 Meters (m/s)   |
| WS50M_MAX   | Maximum Wind Speed at 50 Meters (m/s) |
| WS10M_MAX   | Maximum Wind Speed at 10 Meters (m/s) |
| WS10M_RANGE | Wind Speed Range at 10 Meters (m/s)   |
| PS          | Surface Pressure (kPa)                |
| T2MDEW      | Dew/Frost Point at 2 Meters (ºC)       |
| T2M_MIN     | Minimum Temperature at 2 Meters (ºC)   |
| T2MWET      | Wet Bulb Temperature at 2 Meters (ºC)  |
...
The plotted feature (over three years 2000-2002) looks like this:

![figure_1.png](figures/figure_1.png)  
If we take a look at a smaller period we notice the problem of missing values in our data.  
...
We see that the score column includes 939 missing values. The total number of values in this data frame is 1096, which means that roughly 86% of the data is missing. This means that we can't just delete the rows with missing data (this is a common way to deal with missing data).  
Taking a closer look at the description of the dataset on Kaggle, we can see the reason of the missing values. The score feature is a feature that is only measured once a week. So we have one data point and then 6 missing data points.  
...
With the interpolated values we were able to start trying out different models.
...
In the first stage of our project, the plan was, that everyone tries a different approach on a subset of the whole dataset (the years 2000-2002) so that we can evaluate afterwards which model performs best. After that, we wanted to use the best performing model to train on the whole dataset for the final prediction.  
The models we then tried were Exponential Smoothing, ARIMA, Random Forest Regression, Prophet and an LSTM Neural Network.  
**_Arima_**  
We had some problems with the ARIMA model since we had to differentiate the data since it was not stationary. This is a step you have to take in order to create an ARIMA model<sup>[4]</sup>. The predictions for the differenced data were promising. But if we transformed this data back the predictions weren't that good like the plots show:  
![figure_13.png](figures/figure_13.png)
![figure_14.png](figures/figure_14.png)  
_In the upper plot the blue line is the differenced score value and the other line is the ARIMA prediction._  
_In the bottom plot the blue line is the score value and the other line is the ARIMA prediction._  
On the differenced data the model predicted quite well, but after transforming the data back the predictions were quite bad. 
So we decided to continue with another approach.  
**_Prophet_**  
Prophet is a model which was built by Facebook. It makes time series forecasting simple. You have to provide it with a date column and the target variable column, which you want to predict. We did this, but the problem was that the score variable (our target variable) is in the range from 0 to 5. In the Prophet model, there is no way to set boundaries to the predictions, so the model predicted the following:  
![figure_12.png](figures/figure_12.png)  
The predictions alone make sense, but in the context of our problem, they are not reasonable because there is no Drought Level that matches negative values. We therefore decided to chose a different approach.  
...
This function is using a rule of thumb, by averaging the past results and assigning them to equal weight.   
As in the case of the other approaches, we had a problem with the dataset and its missing values as well. We also tried fixing this problem by applying the interpolation function on the dataset. This did not fix all problems and we decided against this method, because of its lack of precision.  
**_Random Forest_**  
...
A supervised learning problem is a problem where we have one target/dependent variable, which we want to predict based on one or more independent variables.  
...
The next step was to train the model on the whole training set. We imported the test set and made predictions, which you can see here:  
...
For the purpose of easiness we trained, tested and validated the model only for the fips number "1001".  
This approach should work as good for the other fips numbers because before we fitted the model we deleted the fips column. We did this because at this point it didn't have any effect on the data.

## Conclusion

We were able to get some decent results in predicting the drought score for the regions. Although, there is still potential to improve the model, or use other approaches. For example, it is possible to try a Neural Network approach to this problem. On Kaggle there is an example where an LSTM (Long-short term memory) Neural Network was used<sup>[5]</sup>. Another approach could be to instead of a regression model is to use a classification model. So that you feed the meteorological data to the model and it predicts which drought category this would be. This is a great example of data that can be used in different ways.
